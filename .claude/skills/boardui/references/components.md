...
Every installable BoardUI item, generated from the live registry (version 2026.9.11). Names are exact. Free items install with `npx boardui@latest add <name>`; Pro items need a one-time BoardUI Pro license (https://www.boardui.com/pricing) activated once with `npx boardui@latest login <key>`.
